$(function(){
    ...
    $('.section').each(function(index){
        $(this).on('mousewheel DOMMouseScroll', function(event){  //this=section이다  
            event.preventDefault(); //원래값을 차단   
            browser=window.navigator.userAgent.toLowerCase().indexOf('firefox');// 소문자로만들어 파이어폭스가있으면 파이어폭스가 있으면 -1값
            if(browser>=0){
